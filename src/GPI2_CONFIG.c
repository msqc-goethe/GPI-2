...
gaspi_return_t pgaspi_config_set(const gaspi_config_t nconf) {
	...
	{
#ifdef DEBUG
		const char* gaspi_network_str[] = {[GASPI_IB] = "GASPI_IB",
	...
	if (nconf.queue_num > GASPI_MAX_QP || nconf.queue_num < 1) {
		GASPI_DEBUG_PRINT_ERROR(
		    "Invalid value for parameter queue_num (min=1 and "
		    "max=GASPI_MAX_QP");
		return GASPI_ERR_CONFIG;
	}
	...
	if (nconf.segment_max > GASPI_MAX_MSEGS || nconf.segment_max < 1) {
		GASPI_DEBUG_PRINT_ERROR(
		    "Invalid value for parameter segment_max (min=1 and "
		    "max=GASPI_MAX_MSEGS)");
		return GASPI_ERR_CONFIG;
	}
 ...
 passive_queue_size_max");
	...
	if (nconf.passive_transfer_size_max > GASPI_MAX_TSIZE_P) {
		GASPI_DEBUG_PRINT_ERROR(
		    "Invalid value (%u) for parameter passive_transfer_size_max "
		    "(max=%lu)",
 ...
 allreduce_buf_size");
	...
	if (nconf.rw_list_elem_max > GASPI_RW_LIST_ELEM_MAX ||
	    nconf.rw_list_elem_max < 1) {
		GASPI_DEBUG_PRINT_ERROR(
		    "Invalid value for parameter rw_list_elem_max (min=1 and "
		    "max=GASPI_RW_LIST_ELEM_MAX)");
		return GASPI_ERR_CONFIG;
	}
