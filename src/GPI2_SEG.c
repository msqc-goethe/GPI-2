...
gaspi_return_t pgaspi_segment_avail_local(
    gaspi_segment_id_t* const avail_seg_id) {
	gaspi_context_t const* const gctx = &glb_gaspi_ctx;
...
static inline int pgaspi_segment_create_desc(
...
static inline int pgaspi_segment_alloc_maybe(
	...
	size_t const allocation_size = pointer == NULL
	                                   ? size + NOTIFICATIONS_SPACE_SIZE
	                                   : NOTIFICATIONS_SPACE_SIZE;

	int const allocation_failed = pgaspi_alloc_page_aligned(
	    &(myrank_mseg->notif_spc.ptr), allocation_size);
	...
	myrank_mseg->data.ptr =
	    myrank_mseg->user_provided
	        ? pointer
	...
	unsigned char* segPtr = (unsigned char*) myrank_mseg->notif_spc.addr +
	                        NOTIFICATIONS_SPACE_SIZE -
...
gaspi_return_t pgaspi_segment_register(const gaspi_segment_id_t segment_id,
	...
	gaspi_return_t eret = gaspi_sn_command(
	    GASPI_SN_SEG_REGISTER, rank, timeout_ms, (void*) &segment_id);
...
static gaspi_return_t pgaspi_segment_register_group(
    gaspi_context_t* const gctx,
    const gaspi_segment_id_t segment_id,
	...
	if (gaspi_sn_allgather(gctx,
	                       &cdh,
	                       result,
	                       sizeof(gaspi_segment_descriptor_t),
	                       group,
	                       timeout_ms) != 0) {
		free(result);
		unlock_gaspi(&(gctx->ctx_lock));
...
gaspi_return_t pgaspi_segment_create(const gaspi_segment_id_t segment_id,
	...
	if (GASPI_TOPOLOGY_STATIC == gctx->config->build_infrastructure) {
		for (int r = gctx->groups[group].rank; r < gctx->groups[group].tnc;
		     r++) {
...
gaspi_return_t pgaspi_segment_bind(
    gaspi_segment_id_t const segment_id,
    gaspi_pointer_t const pointer,
    gaspi_size_t const size,
...
gaspi_return_t pgaspi_segment_use(
    gaspi_segment_id_t const segment_id,
    gaspi_pointer_t const pointer,
    gaspi_size_t const size,
