...
typedef struct {
	...
	struct {
		struct {
			...
			gaspi_uint
			    port_check; /* flag to whether to perform a network check */
		} ib;
 ...
 *
 */
typedef enum { GASPI_STATE_HEALTHY = 0, GASPI_STATE_CORRUPT = 1 } gaspi_state_t;
...
typedef enum {
	GASPI_TOPOLOGY_NONE =
	    0, /* No connection will be established. GASPI_GROUP_ALL is not set.  */
...
typedef struct gaspi_config {
	...
	gaspi_size_t
	    transfer_size_max; /* maximum size (bytes) of a single data transfer */
	gaspi_number_t
	    notification_num; /* maximum number of allowed notifications */
	gaspi_number_t
	    passive_queue_size_max; /* maximum number of allowed on-going passive requests */
	gaspi_number_t
	    passive_transfer_size_max; /* maximum size (bytes) of a single passive transfer */
	gaspi_size_t
	    allreduce_buf_size; /* size of internal buffer for gaspi_allreduce_user */
	gaspi_number_t
	    allreduce_elem_max; /* maximum number of elements in gaspi_allreduce */
	gaspi_number_t
	    rw_list_elem_max; /* maximum number of elements in gaspi_rw_list */
	gaspi_topology_t
	    build_infrastructure; /* whether and how the topology should be built at initialization */
