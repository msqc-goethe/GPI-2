...
gaspi_return_t pgaspi_version(float* const version) {
...
gaspi_return_t pgaspi_set_socket_affinity(const gaspi_uchar sock) {
	cpu_set_t sock_mask;
...
gaspi_return_t pgaspi_numa_socket(gaspi_uchar* const sock) {
...
static gaspi_return_t pgaspi_create_error_vector(gaspi_context_t* gctx) {
...
static gaspi_return_t pgaspi_init_core(gaspi_context_t* const gctx) {
	if (gctx->dev_init) {
		return GASPI_ERROR;
		;
	}
	...
	gctx->ep_conn = (gaspi_endpoint_conn_t*) calloc(
	    gctx->tnc, sizeof(gaspi_endpoint_conn_t));
...
static int pgaspi_parse_machinefile(gaspi_context_t* const gctx) {
	...
	if (gctx->config->build_infrastructure) {
		...
		if (GASPI_TOPOLOGY_STATIC == gctx->config->build_infrastructure) {
			for (int i = gctx->rank; i >= 0; i--) {
				if ((eret = pgaspi_connect((gaspi_rank_t) i, timeout_ms)) !=
				    GASPI_SUCCESS) {
					return eret;
				}
			}
...
gaspi_return_t pgaspi_build_infrastructure(gaspi_number_t* build) {
...
static gaspi_return_t pgaspi_cleanup_core(gaspi_context_t* const gctx) {
	if (!(gctx->dev_init)) {
		return GASPI_ERROR;
	}
	...
	if (gctx->num_queues != gctx->config->queue_num) {
		for (gaspi_uint q = gctx->config->queue_num; q < gctx->num_queues;
		     q++) {
...
gaspi_return_t pgaspi_proc_term(const gaspi_timeout_t timeout) {
	gaspi_context_t* const gctx = &glb_gaspi_ctx;
	...
	if (gctx->rank == 0) {
		if (remove(gctx->mfile) < 0) {
			GASPI_DEBUG_PRINT_ERROR("Failed to remove tmp file (%s)",
			                        gctx->mfile);
		}
	}
	//#endif
...
gaspi_return_t pgaspi_proc_ping(const gaspi_rank_t rank,
...
gaspi_string_t pgaspi_error_str(gaspi_return_t error_code) {
	static const char* gaspi_return_str[] = {
	    ...
	    [GASPI_ERR_CONFIG] =
	        "Invalid parameter in configuration (gaspi_config_t)",
...
gaspi_return_t pgaspi_print_error(gaspi_return_t error_code,
